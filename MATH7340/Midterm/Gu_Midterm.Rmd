...
### Problem 6 (35 points)
##### On the Golub et al. (1999) data set, compare the 鈥淕RO3 GRO3 oncogene鈥? (at row 2715) with the 鈥淢YC V-myc avian myelocytomatosis viral oncogene homolog鈥? (at row 2302).
```{r}
data(golub)
```
...
### Problem 7 (15 points)
##### On the Golub et al. (1999) data set, complete the following:
##### a) Find the row number of the 鈥淗PCA Hippocalcin鈥? gene.
```{r}
data(golub)
grep("HPCA Hippocalcin", golub.gnames[,2])
```
##### b) Find the proportion among ALL patients that the 鈥淗PCA Hippocalcin鈥? gene is negatively expressed (expression value<0).
...
##### c) We want to show that 鈥淗PCA Hippocalcin鈥? gene is negatively expressed in at least half of the population of the ALL patients. State the null hypothesis and the alternative hypothesis. Carry out the appropriate test.
...
Since the p-value here is greater than 0.05, we reject the alternative hypothesis that $p_{ALL,HPCA}<0.5$. Thus, 鈥淗PCA Hippocalcin鈥? gene is negatively expressed in at least half of the population of the ALL patients.

##### d) Find a 95% confidence interval for the difference of proportions in the ALL group versus in the AML group of patients with negatively expressed 鈥淗PCA Hippocalcin鈥? gene.
